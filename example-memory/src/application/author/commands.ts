...
export class UpdateAuthorCommand extends Action {
    public async execute({ name, email, id }: IUpdateAuthorDto) {
        ...
        const updatedAuthor = await this.dataContext.Author.update(id, author);

        if (!updatedAuthor) return new MediatorResultFailure(ACTION_RESULT.ERROR_INTERNAL_ERROR);
